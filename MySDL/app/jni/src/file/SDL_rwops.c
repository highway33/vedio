...
{
  LOGE("SDL_RWFromFile:%s",file);
    SDL_RWops *rwops = NULL;
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "SDL_RWFromFile123");
    if (!file || !*file || !mode || !*mode) {
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "define file faile");
        SDL_SetError("SDL_RWFromFile(): No file or no mode specified");

        return NULL;
    }
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "define file succuss");
#if defined(__ANDROID__)
#ifdef HAVE_STDIO_H
    /* Try to open the file on the filesystem first */
    if (*file == '/') {
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "file system");
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "relative");
        char *path;
        FILE *fp;
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "asset");
    rwops = SDL_AllocRW();
    if (!rwops)
