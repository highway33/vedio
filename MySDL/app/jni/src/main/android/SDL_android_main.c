...
JNIEXPORT int JNICALL Java_org_libsdl_app_SDLActivity_nativeInit(JNIEnv* env, jclass cls, jobject array)
{
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "main");
    ...
    len = (*env)->GetArrayLength(env, array);
     __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "arg:%d",len);
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (string) {
            ...
            __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "arg:%s",*utf);
            if (utf) {
                arg = SDL_strdup(utf);
 ...
 __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "status:%d",status);
